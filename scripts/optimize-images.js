...
async function optimizeImage(filePath) {
  try {
    ...
    console.log(
      `🔄 Processing ${fileName} (${(originalSize / 1024 / 1024).toFixed(2)} MB, ${metadata.width}x${metadata.height})`
    )
    ...
    if (ext === '.png') {
      ...
      await pipeline.jpeg({ quality: QUALITY, progressive: true, mozjpeg: true }).toFile(tempPath)
    }
    ...
    if (newSize < originalSize) {
      ...
      console.log(
        `✅ ${fileName}: ${(originalSize / 1024 / 1024).toFixed(2)} MB → ${(newSize / 1024 / 1024).toFixed(2)} MB (saved ${savedPercent}%)`
      )
    } else {
